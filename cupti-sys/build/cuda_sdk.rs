use std::{env, error, ffi, fs, iter, path};
...
impl CudaSdk {
    ...
    ///
    /// # Errors
    /// Returns an error if the CUDA SDK cannot be found or if the versions
    /// cannot be parsed.
    ...
    fn parse_cuda_library_path_env() -> Vec<path::PathBuf> {
        // The location of the libcuda, libcudart, libcublas, etc. can be hardcoded with
        // the CUDA_LIBRARY_PATH environment variable.
    ...
    /// Parses the content of the `cuda.h` header file to extract the driver
    /// version.
    ///
    /// # Errors
    /// Returns an error if the `CUDA_VERSION` definition cannot be found or
    /// parsed.
